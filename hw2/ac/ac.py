...
class ACAgent:
    ...
    def update_critic(self, replay_iter):
        ...
        with torch.no_grad():
            ...
            next_action = next_action_dist.mean
            next_state_qs = self.critic_target(next_obs, next_action)
        
            # Compute the Bellman targets
            indices = torch.randperm(len(next_state_qs))[:2]
            q1, q2 = next_state_qs[indices[0]], next_state_qs[indices[1]]
            target_q = torch.minimum(q1, q2)
            critic_target = reward + discount * target_q.squeeze(-1)

        # Compute the loss, it's a list of loss for each critic
        losses = [(critic_target - current_qs[i].squeeze(-1)).pow(2).mean() for i in range(len(current_qs))]

        # Take a gradient step with respect to the critic parameters
        total_loss = sum(losses) / len(losses)   # or just sum(losses)
        ...
        metrics['target_q_mean'] = critic_target.mean().item()
        #####################
        return metrics
    ...
    def update_actor(self, replay_iter):
        ...
        ### YOUR CODE HERE ###
        # Sample an action from the actor a′t ∼ πθ(ot)
        dist = self.actor(obs)
        sampled_action = dist.sample()
        # Loss function is -Q(s, a')
        qs = self.critic(obs, sampled_action)
        qs_1d = [q.squeeze(-1) for q in qs]
        loss = -1.0 * sum([q.float().mean() for q in qs_1d])
    ...
    def bc(self, replay_iter):
        ...
        loss = -log_probs.mean()            # scalar for .backward()
